...
def remove_outlier_data(data: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in outliers_15iqr.iterrows():
        data_without_outliers = data_without_outliers[
            (data_without_outliers[row[0]] >= row[1]["lower_boundary"])
...
def standardize_train_set(
    data_train: pd.DataFrame, column_names: List[str]
) -> pd.DataFrame:
...
def standardize_test_set(
    x_train: pd.DataFrame, data_test: pd.DataFrame, column_names: List[str]
) -> pd.DataFrame:
